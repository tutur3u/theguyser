...
export function PortfolioHeader({
  ...
  return (
    <header className="relative sticky top-0 z-10 mx-2 mt-2 rounded-[1.6rem] border border-white/60 bg-white/82 px-3 py-2 shadow-[0_14px_35px_rgba(15,23,42,0.08)] backdrop-blur-xl dark:border-white/10 dark:bg-gray-900/84 md:mx-6 md:mt-4 md:rounded-[2rem] md:px-8 md:py-3">
      <div className="flex items-center justify-between gap-3 md:gap-6">
        ...
        <div className="flex shrink-0 items-center gap-2 text-sm font-bold text-gray-600 dark:text-gray-300 md:gap-6 md:text-base">
          ...
          <div className="flex items-center gap-2 rounded-full bg-gray-100/90 px-2.5 py-1.5 dark:bg-gray-800/85 md:gap-3 md:px-3 md:py-2">
            <Wifi className="h-3.5 w-3.5 text-gray-500 dark:text-gray-400 md:h-5 md:w-5" />
            <span className="min-w-[4.6rem] whitespace-nowrap text-right text-sm tabular-nums md:min-w-[5.5rem]">{time}</span>
            <BatteryFull className="h-4.5 w-4.5 text-green-500 dark:text-green-400 md:h-6 md:w-6" />
          </div>
        </div>
      </div>
    ...
    <AnimatePresence>
      {activeApp ? (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-50 flex items-stretch justify-center bg-black/60 p-1.5 backdrop-blur-md sm:items-center sm:p-6"
          role="dialog"
          aria-modal="true"
        ...
        >
          <motion.div
            ...
            className="relative h-[calc(100dvh-0.75rem)] w-full max-w-[calc(100vw-0.75rem)] overflow-hidden rounded-[2rem] bg-[linear-gradient(180deg,#374151_0%,#111827_42%,#0f172a_100%)] shadow-[0_35px_90px_rgba(0,0,0,0.65)] sm:h-[92vh] sm:max-w-[min(1500px,96vw)] sm:rounded-[3.25rem]"
          ...
          >
            ...
            <div className="relative z-10 mx-1.5 mb-3 mt-1.5 flex h-[calc(100%-0.75rem)] min-h-0 flex-col overflow-hidden rounded-[1.75rem] bg-white shadow-[inset_0_0_30px_rgba(0,0,0,0.18)] ring-1 ring-black dark:bg-gray-900 sm:mx-3 sm:mb-24 sm:mt-3 sm:h-[calc(100%-6rem)] sm:rounded-[3rem]">
              <div className={`flex items-center justify-between gap-3 border-b border-gray-200 p-3 sm:px-6 sm:py-4 ${activeApp.color}`}>
                <div className="flex min-w-0 items-center gap-3">
                  <activeApp.icon className="h-7 w-7 text-white drop-shadow-sm sm:h-8 sm:w-8" />
                  <h2 id="tablet-dialog-title" className="truncate text-lg font-black tracking-wide text-white drop-shadow-sm sm:text-2xl">
                    {activeApp.title}
                  </h2>
                </div>
                <button
                  type="button"
                  onClick={onClose}
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/20 font-black text-white backdrop-blur-sm transition-colors hover:bg-white/40"
