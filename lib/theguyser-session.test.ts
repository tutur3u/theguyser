...
import type { TheGuyserAdminSession } from "./theguyser-session";
...
function createSession(): TheGuyserAdminSession {
  return {
...
function readSessionCookieValue(response: NextResponse) {
  ...
  expect(setCookie).toContain("theguyser_admin_session=");
  return setCookie?.split(";")[0]?.slice("theguyser_admin_session=".length) ?? "";
}

describe("theguyser session validation", () => {
  beforeEach(() => {
    process.env.TUTURUUU_API_BASE_URL = "https://platform.example.com/api/v1";
    process.env.TUTURUUU_THEGUYSER_WORKSPACE_ID = "ws-linked";
    process.env.THEGUYSER_SESSION_SECRET = "session-secret";
  ...
  afterEach(() => {
    ...
    delete process.env.TUTURUUU_THEGUYSER_WORKSPACE_ID;
    delete process.env.THEGUYSER_SESSION_SECRET;
    sessionCookieValue = null;
  });

  for (const status of [401, 403, 404] as const) {
    test(`rejects stored sessions when platform revalidation returns ${status}`, async () => {
      const { getTheGuyserSessionFromCookies, setTheGuyserSessionCookie } =
        await import("./theguyser-session");
      const response = NextResponse.json({});
      setTheGuyserSessionCookie(response, createSession());
      ...
      await expect(getTheGuyserSessionFromCookies()).resolves.toBeNull();
