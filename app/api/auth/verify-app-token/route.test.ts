...
describe("theguyser app token verification", () => {
  beforeEach(() => {
    process.env.TUTURUUU_API_BASE_URL = "https://platform.example.com/api/v1";
    process.env.THEGUYSER_APP_SECRET = "app-secret";
  ...
  afterEach(() => {
    ...
    delete process.env.THEGUYSER_APP_SECRET;
  ...
  test("sends the configured linked workspace id during exchange", async () => {
    ...
    expect(response.headers.get("set-cookie")).toContain("theguyser_admin_session");
