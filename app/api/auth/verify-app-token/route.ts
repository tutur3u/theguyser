...
import {
  ...
  type TheGuyserAdminSession,
} from "@/lib/theguyser-session";
...
function toTheGuyserSession(payload: AppTokenExchangeResponse): TheGuyserAdminSession {
  ...
  return {
    ...
    app: {
      name: payload.app?.name ?? getTheGuyserAppId(),
    },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const session = toTheGuyserSession(await exchangeCrossAppToken(token));
    ...
    setTheGuyserSessionCookie(response, session);
    return response;
  } catch (error) {
    console.error("[theguyser:auth] app token exchange failed", error);
